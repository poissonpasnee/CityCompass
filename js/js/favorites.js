const Favorites = {
    render: () => {
        ...
        if(favs.length === 0) {
            list.innerHTML = "<p style='text-align:center; opacity:0.5; margin-top:50px;'>Aucun favori enregistré.</p>";
            return;
        }
        ...
        favs.forEach((f, idx) => {
            ...
            item.innerHTML = `
                <div onclick="MapManager.showPopup({lat:${f.lat}, lng:${f.lng}}, '${f.name.replace(/'/g, "\\'")}')" style="flex:1;">
